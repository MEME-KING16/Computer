...
/**
 * @deprecated Use `Interpreterv2` instead.
 */
class Interpreterv1 {
    constructor() {
        this.variables = {};
...
class Interpreterv2 {
    ...
    handleIf(tokens) {
        const ifPattern = /if\s+([^{]+)\s*\{\s*([^}]*)\s*\}(?:\s*else\s*\{\s*([^}]*)\s*\})?/;
        const match = tokens.join(' ').match(ifPattern);

        if (!match) {
            throw new Error('Invalid if syntax. Usage: if <condition> { <true_block> } else { <false_block> }');
        }

        const [_, condition, trueBlock, falseBlock] = match;
        const conditionResult = this.evaluateExpression(condition.trim());

        if (conditionResult) {
            this.evaluate(this.parse(trueBlock));
        } else if (falseBlock) {
            this.evaluate(this.parse(falseBlock));
        }
    }
    ...
    evaluateOperation(left, operator, right) {
        if (Array.isArray(left) && Array.isArray(right)) {
            switch (operator) {
                ...
                default:
                    throw new Error(`Unknown operator for arrays: ${operator}`);
            }
        }
        ...
        if (typeof left === 'object' && typeof right === 'object') {
            switch (operator) {
                case '+':
                    return { ...left, ...right };
                default:
                    throw new Error(`Unknown operator for objects: ${operator}`);
            }
        }
